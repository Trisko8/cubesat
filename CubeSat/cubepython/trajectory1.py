...
fichero=np.loadtxt("xyz_1",dtype='float',comments='#',delimiter=None, skiprows=0, usecols=None, encoding="utf8")
...
linepts = vv[0] * np.mgrid[-80:80:2j][:, np.newaxis]
...
err_abs = np.absolute(position-fichero)
print(err_abs)
err_rel = np.absolute(np.divide(err_abs, position))
print(err_rel)
    
#Angle 

delta_a=np.sqrt(np.sum(np.square(np.multiply(np.divide(fichero[:,1], np.square(fichero[:,0])+np.square(fichero[:,1])), err_rel[:, 0]))) + np.sum(np.square(np.multiply(np.divide(fichero[:,1], np.square(fichero[:,0])+np.square(fichero[:,1])), err_rel[:, 1]))))
delta_e=np.sqrt(np.sum(np.square(np.multiply(np.divide(fichero[:,1], np.square(fichero[:,2])+np.square(fichero[:,1])), err_rel[:, 1]))) + np.sum(np.square(np.multiply(np.divide(fichero[:,2], np.square(fichero[:,2])+np.square(fichero[:,1])), err_rel[:, 2]))))

a = np.degrees(np.arctan(d[0]/d[1]))
e = np.degrees(np.arctan(d[2]/d[1]))

print("Azimuth = " + str("%.2f" % a) + u"\u00B1" + str("%.2f" % delta_a))
print("Elevation = " + str("%.2f" % e) + u"\u00B1" + str("%.2f" % delta_e))
...
ax.plot3D(*linepts.T, label="P = " + str("[") + str("%.2f" % p0[0]) + str(", ") + str("%.1f" % p0[1]) + str(", ") + str("%.2f" % p0[2]) + str("]") +'+' +'t*' + str("[") + str("%.2f" % d[0]) + str(", ") + str("%.1f" % d[1]) + str(", ") + str("%.1f" % d[2]) + str("]"))
ax.legend()
ax.set_xlabel('$x$' + ' (mm)')
ax.set_ylabel('$y$' + ' (mm)')
ax.set_zlabel('$z$' + ' (mm)')
for i in np.arange(0, len(fichero)):
    ax.plot([fichero[i, 0]+err_rel[i, 0], fichero[i, 0]-err_rel[i, 0]], [fichero[i, 1], fichero[i, 1]], [fichero[i, 2], fichero[i, 2]], marker="_")
    ax.plot([fichero[i, 0], fichero[i, 0]], [fichero[i, 1]+err_rel[i, 1], fichero[i, 1]-err_rel[i, 1]], [fichero[i, 2], fichero[i, 2]], marker="_")
    ax.plot([fichero[i, 0], fichero[i, 0]], [fichero[i, 1], fichero[i, 1]], [fichero[i, 2]+err_rel[i, 2], fichero[i, 2]-err_rel[i, 2]], marker="_")
