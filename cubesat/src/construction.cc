...
void MyDetectorConstruction::DefineMaterials()
{
    ...
    G4double energy[2] = {1.239841939*eV/0.425, 1.239841939*eV/0.425};

    G4MaterialPropertiesTable *mptScintillator = new G4MaterialPropertiesTable();
    G4double rindex[2] = {1.58, 1.58};
    mptScintillator->AddProperty("RINDEX", energy, rindex, 2);

    G4double fraction[2] = {1.0, 1.0}; 
    mptScintillator->AddProperty("FASTCOMPONENT", energy, fraction, 2); //Photons for each wavelength intervalue

    mptScintillator->AddConstProperty("SCINTILLATIONYIELD", 10./keV); //Photons per energy loss of the particle created
